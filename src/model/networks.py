...
# todo: Weight init (wagi sieci inicjalizowane do gaussa z variancją 0.2 i średnią 0 dla conv i 1 dla batchNorm
...
def completion_net(input, n_downsample=3, channels=64, norm_layer=tf.contrib.layers.instance_norm):
    ...
    *CompGenerator*
    ...
    :return:
    """
    with tf.variable_scope('completion_net'):
        x = input
        x = reflectionPadding2D(x, 3)
        x = tf.layers.conv2d(x, filters=channels, kernel_size=(7, 7))
        x = norm_layer(x)
        x = tf.nn.relu(x)

        for i in range(n_downsample):
            mult = 2 ** i
            x = tf.layers.conv2d(x, filters=channels * mult * 2, kernel_size=(3, 3), strides=(2, 2), padding='same')
            x = norm_layer(x)
            x = tf.nn.relu(x)

        x = reflectionPadding2D(x, 3)
        x = tf.layers.conv2d(x, filters=3, kernel_size=(3, 3))
        x = tf.nn.tanh(x)
    return x
...
def fine_net(input, n_downsample=3, channels=64, n_blocks=9, norm_layer=tf.contrib.layers.instance_norm):
    ...
    :return:
    """
    with tf.variable_scope('fine_net'):
        x = input
        x = reflectionPadding2D(x, 3)
        x = tf.layers.conv2d(x, filters=channels, kernel_size=(7, 7))
        ...
        for i in range(n_downsample):
            ...
            x = tf.layers.conv2d_transpose(x, filters=filters, kernel_size=(3, 3), strides=(2, 2), padding='same')
            x = norm_layer(x)
            x = tf.nn.relu(x)

        x = reflectionPadding2D(x, 3)
        x = tf.layers.conv2d(x, filters=3, kernel_size=(7, 7))
        x = tf.nn.tanh(x)
    return x
...
def multiscale_discriminator(input, n_discriminators=2, channels=64, n_layers=3,
    ...
    """
    with tf.variable_scope("multiscale_discriminator"):
...
def n_layer_discriminator(input, channels, n_layers, norm_layer):
    ...
    with tf.variable_scope('input_block'):
        x = input
        x = tf.layers.conv2d(x, filters=channels, kernel_size=(4, 4), strides=(2, 2), padding='same')
    ...
    for n in range(1, n_layers):
        with tf.variable_scope('layer_' + str(n)):
            ...
            x = tf.layers.conv2d(x, filters=filters, kernel_size=(4, 4), strides=(2, 2), padding='same')
            x = norm_layer(x)
            x = tf.nn.leaky_relu(x, alpha=0.2)
            feature_layers.append(x)
    ...
    with tf.variable_scope('output_block'):
        ...
        x = tf.layers.conv2d(x, filters=filters, kernel_size=(4, 4), padding='same')
        x = norm_layer(x)
        x = tf.nn.leaky_relu(x, alpha=0.2)
        feature_layers.append(x)

        x = tf.layers.conv2d(x, filters=1, kernel_size=(4, 4), padding='same', name='final_conv')
...
def resnet_block(features, channels, norm_layer):
    ...
    c = tf.layers.conv2d(c, filters=channels, kernel_size=(3, 3))
    c = norm_layer(c)
    c = tf.nn.relu(c)
